...
// GET /api/v2/inbox — own inbox messages only
...
// GET /api/v2/inbox/download/:filename — it_staff / admin only
...
// GET /api/v2/inbox/:id — own messages only
router.get("/:id", verifyToken, (req, res) => {
  const db  = new Database(DB_PATH);
  const msg = db.prepare(
    "SELECT * FROM inbox WHERE id = ? AND recipient_id = ?"
